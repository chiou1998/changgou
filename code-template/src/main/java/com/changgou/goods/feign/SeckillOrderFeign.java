...
public interface SeckillOrderFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) SeckillOrder seckillOrder, @PathVariable(name="page")  int page, @PathVariable(name="size")  int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody SeckillOrder seckillOrder,@PathVariable(name="id") Long id);
     ...
     */
    @GetMapping("/{id}")
    Result<SeckillOrder> findById(@PathVariable(name="id") Long id);
