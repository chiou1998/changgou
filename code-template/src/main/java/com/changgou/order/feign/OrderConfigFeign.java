...
public interface OrderConfigFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) OrderConfig orderConfig, @PathVariable(name="page")  int page, @PathVariable(name="size")  int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody OrderConfig orderConfig,@PathVariable(name="id") Integer id);
     ...
     */
    @GetMapping("/{id}")
    Result<OrderConfig> findById(@PathVariable(name="id") Integer id);
