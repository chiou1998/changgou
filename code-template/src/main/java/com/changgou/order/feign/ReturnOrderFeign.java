...
public interface ReturnOrderFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) ReturnOrder returnOrder, @PathVariable(name="page")  int page, @PathVariable(name="size")  int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody ReturnOrder returnOrder,@PathVariable(name="id") Long id);
     ...
     */
    @GetMapping("/{id}")
    Result<ReturnOrder> findById(@PathVariable(name="id") Long id);
