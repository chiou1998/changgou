...
public interface SeckillGoodsFeign {
    ...
    Result<PageInfo> findPage(@RequestBody(required = false) SeckillGoods seckillGoods, @PathVariable(name="page")  int page, @PathVariable(name="size")  int size);
     ...
     */
    @PutMapping(value="/{id}")
    Result update(@RequestBody SeckillGoods seckillGoods,@PathVariable(name="id") Long id);
     ...
     */
    @GetMapping("/{id}")
    Result<SeckillGoods> findById(@PathVariable(name="id") Long id);
